...
public class DecodeFrame
{
    ...
    public int           DEFAULT_FRAMES_MAX_NUMBER = 10;
    public int           DEFAULT_TRIES_MAX_NUMBER  = 2;
    ...
    public int compute()
    {
        ...
        for (int index = 0; index < numberOfRolls; index++)
        {
                ...
                if (triesCount == 0)
                {
                    ...
                    } else
                    {
                        if (isPendingStrike > 0)
                        {
                            ...
                            if (frameCounter > DEFAULT_FRAMES_MAX_NUMBER)
                            {
                                System.out.println("Frame is : " + frameCounter + " - Pending strike set to 1");
                                pendingStrike = 1;
                            }
            ...
            if (aRoll.isFrameComplete())
            {
                ...
                score += registerFrameScore(frameScore, aRoll.isSpare(), aRoll.isStrike());
            ...
            {
                frameCounter++;
                System.out.println("Frame : " + frameCounter + " is closed");
            } else
            {
                System.out.println("Try count : " + triesCount + " of " + DEFAULT_TRIES_MAX_NUMBER);
            }
    ...
    private int registerFrameScore(int frameScore, boolean isSpare, boolean isStrike)
    {
        ...
        if (frameCounter < DEFAULT_FRAMES_MAX_NUMBER)
        {
            ...
            if (frameScore > DecodeRoll.MAX_PIN_NUMBER)
            {
                ...
                } else
                {
                    ...
                    // If we want to make it simpler, we can remove this rule
                    // and
                    // return DecodeRoll.MAX_PIN_NUMBER
                    // But let have some fun!!!
                    System.out.println("Cheater? Only the big lebowski can do this!!!");
                    return 0;
                }
            } else
            {
        ...
        {
            System.out.println("Register score - Temporary score not added : " + frameScore);
            return 0;
        }
    }
}
